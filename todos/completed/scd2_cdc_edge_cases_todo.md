...
|----|----------|----------|-------|--------|
| E-1 | P0 | Hash Hygiene | Oracle empty string = NULL phantom updates | DONE — `add_row_hash(source_is_oracle=True)` normalizes '' → NULL |
| E-2 | P0 | SCD2 Integrity | INSERT-first vs filtered unique index conflict | DONE — operation='U' inserts with UdmActiveFlag=0, activation UPDATE after close |
| E-3 | P0 | SCD2 Integrity | BCP `-b` flag breaks SCD2 atomicity | DONE — `bcp_load(atomic=True)` default; Stage/staging use `atomic=False` |
| E-4 | P0 | Hash Hygiene | Trailing space divergence phantom hashes | DONE — RTRIM on all string cols before hashing |
| E-5 | P0 | SCD2 Integrity | UPDATE FROM JOIN staging dedup (defense-in-depth) | DONE — `.unique(subset=pk_columns)` in close concat + `_execute_bronze_updates` |
| E-6 | P1 | SCD2 Logic | Resurrection pattern ordering | DONE — defense-in-depth in `run_scd2()` + targeted resurrection filter in `run_scd2_targeted()` |
| E-7 | P1 | Reconciliation | Full-PK reconciliation for windowed delete drift | DONE — `reconcile_active_pks()` in reconciliation.py with ghost/missing PK detection |
| E-8 | P1 | Concurrency | RCSI transient inconsistency window documentation | DONE — documented in scd2/engine.py docstring + CLAUDE.md |
| E-9 | P1 | Performance | UPDATE TOP(500K) lock escalation + RCSI verification | DONE — `verify_rcsi_enabled()` in connections.py, called from both main entry points |
| E-10 | P1 | Operations | Transaction log sizing for large UPDATEs | DONE — sizing estimates + `_check_log_space()` pre-flight in `_execute_bronze_updates()` |
| E-11 | P1 | Schema Safety | Pre-extraction schema validation | DONE — `validate_source_schema()` in evolution.py, called from small_tables.py + large_tables.py |
| E-12 | P2 | Monitoring | Phantom change rate alerting | DONE — update_ratio in CDC_PROMOTION metadata + >50% warning in both orchestrators |
| E-13 | P2 | Monitoring | SCD2 version velocity tracking | DONE — `check_version_velocity()` in reconciliation.py with avg/max/high-velocity PK counts |
| E-14 | P2 | Monitoring | Active-to-total ratio monitoring | DONE — active_ratio in SCD2_PROMOTION metadata + <1% warning in both orchestrators |
| E-15 | P2 | Monitoring | Data freshness alerting | DONE — `_log_data_freshness()` warns when max UdmEffectiveDateTime >48 hours stale |
| E-16 | P2 | Monitoring | Cross-platform type drift detection | DONE — `detect_source_type_drift()` in column_sync.py compares source vs Stage precision/scale |
| E-17 | P2 | Reconciliation | Daily aggregate column reconciliation | DONE — `reconcile_aggregates()` in reconciliation.py compares SUM/COUNT/MIN/MAX of numeric cols |
| E-18 | P3 | SCD2 Logic | Re-insert version chain (audit trail) | DONE — UdmScd2Operation='R' for resurrected PKs in both run_scd2() + run_scd2_targeted(), _activate_new_versions() targets 'U' and 'R' |
| E-19 | P3 | Hash Hygiene | Column delimiter verification (already addressed) | DONE — documented \x1F cross-column collision prevention in add_row_hash() + add_row_hash_fallback() |
| E-20 | P3 | Hash Safety | Categorical column hash safety check | DONE — Categorical dtype detection + cast to Utf8 in add_row_hash() + add_row_hash_fallback() |
| E-21 | P3 | Extraction | Oracle NUMBER precision rounding order | DONE — documented ConnectorX float64 precision loss for >15 significant digits in add_row_hash() + CLAUDE.md |
