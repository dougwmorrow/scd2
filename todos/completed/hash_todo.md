...
## 1. ~~Critical — Data Corruption (P0)~~ → RESOLVED

### B-1 (RESOLVED — NOT P0): 64-bit hash truncation — birthday paradox does NOT apply to per-PK CDC

**Files:** `bcp_csv.py` (`add_row_hash`, `add_row_hash_fallback`), `engine.py` (CDC comparison), Bronze schema

**Resolution:** Dedicated research ("64-Bit Hashes Are Safe for CDC: Why the Birthday Paradox Does Not Apply to Per-Key Change Detection") conclusively demonstrates that the ~24.4% collision probability cited in the original B-1 assessment **answers the wrong question**. The birthday paradox computes the probability that *any two rows in the entire dataset* share a hash — but CDC never performs that comparison. Each primary key's new hash is compared only against that same key's previous hash, making this a **second-preimage problem (2^b security)** not a **collision problem (2^(b/2) security)**.

**The math:**
- **Birthday paradox (incorrectly applied):** P ≈ (3×10⁹)²/(2×2⁶⁴) ≈ 24.4% — counts ~4.5×10¹⁸ cross-PK pairs the pipeline never evaluates
- **Per-PK sequential model (correct for CDC):** P ≤ 2×10⁹/2⁶⁴ ≈ **1.08×10⁻¹⁰** — one false negative expected every ~10,000 years of daily runs
- The global model overestimates by a factor of approximately **one billion**

**Why the original assessment was wrong:** The sources cited (Daniel Lemire, John D. Cook, Jeff Preshing, Scalefree Data Vault community) all analyze *global uniqueness* scenarios where the birthday paradox genuinely applies — hash-based surrogate keys, deduplication, identifier assignment. None of them discuss per-PK change detection. Their math was correctly computed but incorrectly applied to our CDC pipeline. The only practitioner found to explicitly articulate the per-PK distinction is ngalemmo on the Kimball Group Forum (2012), who argued even 32-bit hashing is acceptable for change detection.

**⚠️ This reverses the B-1 assessment and restores the original validated architecture decision: "SHA-256 truncated to 64-bit is safe for per-PK CDC comparison (birthday paradox doesn't apply)." That original assessment was correct all along.**

**Where 64-bit genuinely IS unsafe (birthday paradox applies):**
- Hash-based surrogate keys for joining (~24.4% collision at 3B rows — catastrophic)
- Full-table deduplication or cross-PK reconciliation
- Hash indexes used for global lookups
- Any future repurposing of the hash column as a join key or dedup key

**Current status:** The migration to full SHA-256 VARCHAR(64) was already implemented (`migrations/b1_hash_varchar64.py`). While the migration was not strictly necessary for CDC safety, it provides a belt-and-suspenders defense: it eliminates the need to reason about which collision model applies if the hash column is ever repurposed, and aligns with Scalefree's 128-bit minimum recommendation for Data Vault hash keys. **No rollback recommended** — the VARCHAR(64) implementation is operationally sound and removes all future ambiguity.
...
|----|----------|----------|-------|--------|
| B-1 | ~~P0~~ RESOLVED | Hash Integrity | 64-bit hash safe for per-PK CDC — birthday paradox was incorrectly applied | DONE — VARCHAR(64) migration kept as belt-and-suspenders; 64-bit was safe all along for CDC |
...
1. **~~B-1 (P0):~~** RESOLVED — 64-bit hash was safe for per-PK CDC all along. Birthday paradox applies to global uniqueness, not per-key change detection. Per-PK collision risk is ~10⁻¹⁰, not 24.4%. VARCHAR(64) migration kept as defense-in-depth.
...
- **B-1 (64-bit hash safety) — REVERSED:** The B-series research originally assessed that the birthday paradox made 64-bit hashing a P0 showstopper at billion-row scale. Dedicated follow-up research ("64-Bit Hashes Are Safe for CDC") demonstrates this assessment was **incorrect** — the birthday paradox applies to global uniqueness (surrogate keys, deduplication) but not to per-PK change detection, which is a second-preimage problem with 2^64 security. The *original* validated architecture decision ("SHA-256 truncated to 64-bit is safe for per-PK CDC") was correct all along. Per-PK collision probability at 3B rows is ~1.6×10⁻¹⁰ (one in six billion), not 24.4%. The VARCHAR(64) migration is retained as defense-in-depth but was not required for CDC correctness.

### Validated architecture decisions (confirmed by this research):

- **SHA-256 truncated to 64-bit is safe for per-PK CDC comparison** — the birthday paradox does not apply. Per-PK collision risk is ~1.6×10⁻¹⁰ at 3B rows (second-preimage resistance, 2^b security), not the 24.4% global collision probability (birthday bound, 2^(b/2) security). Confirmed by dedicated research cross-referencing Lemire, Cook, Preshing, Scalefree, and cryptographic second-preimage theory. Note: 64-bit is NOT safe for hash-based surrogate keys, deduplication, or any operation requiring global uniqueness.
...
*Sources: "Validating a CDC/SCD2 Pipeline at Billion-Row Scale: Hash Collision Risk, Lock Escalation, and Architectural Best Practices" and "64-Bit Hashes Are Safe for CDC: Why the Birthday Paradox Does Not Apply to Per-Key Change Detection" — validated against Daniel Lemire, John D. Cook, Preshing birthday tables, Scalefree Data Vault community, Brent Ozar, SQLServerCentral, MSSQLTips, Uber UDQ, DQOps, Acceldata, Aaron Bertrand, Paul White, Hugo Kornelis, Michael J. Swart, dbt, Databricks, Snowflake, Netflix, Debezium/Zalando post-mortems, Auth0 birthday attack analysis, ngalemmo (Kimball Group Forum), and cryptographic collision vs. second-preimage resistance theory.*